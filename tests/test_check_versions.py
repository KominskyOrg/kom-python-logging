...
from workflow_scripts.check_versions import get_current_version, get_latest_pypi_version, compare_versions, main
...
def test_get_current_version_no_version(mocker):
    ...
    with pytest.raises(RuntimeError, match="Cannot find the version information in setup.py"):
        get_current_version()
...
def test_get_latest_pypi_version_error(mocker):
    ...
    package_name = "kom-python-logging"
    with pytest.raises(RuntimeError, match="Error fetching package information from PyPI: 404"):
...
def test_main(mocker):
    ...
    main()
    mock_print.assert_called_with("Current version 0.5 is greater than the latest PyPI version 0.4")
...
def test_script_execution(mocker):
    ...
    result = subprocess.run([sys.executable, "workflow_scripts/check_versions.py"], capture_output=True, text=True)

    assert result.returncode == 0
    assert "Current version 0.5 is greater than the latest PyPI version 0.4" in result.stdout
